...
function renderProfiles() {
  ...
  if (!savedProfiles.length) {
    savedProfilesEl.innerHTML = '<p class="muted compact">No saved profiles yet.</p>';
    return;
  }
...
function renderRounds(rounds, currentRoundId) {
  const signature = `${currentRoundId || ""}:${rounds
    .map((round) => `${round.id}:${round.open}:${round.winner_profile_id || ""}`)
    .join("|")}`;
  ...
  roundsList.innerHTML = rounds
    .map((round) => {
      ...
      const winnerLabel = round.winner_profile_id ? `Winner: ${round.winner_profile_id}` : "No winner";
...
savedProfilesEl.addEventListener("click", async (event) => {
  ...
  const savedProfile = savedProfiles.find((profile) => profile.id === profileId);
...
const realtimeUrl = `ws://localhost:54324/socket/websocket?apikey=${encodeURIComponent(SUPABASE_ANON_KEY)}&vsn=1.0.0`;
...
async function bootstrapSession() {
  const savedProfiles = readSavedProfiles();
  const preferredProfile = savedProfiles.find((profile) => profile.id === currentProfileId) || savedProfiles[0];
